  ...
  return (
    ...
    >
      <Container fluid style={{ maxWidth: "1200px", paddingLeft: 0, paddingRight: 0 }}>
        ...
        <h2 className="mb-4 text-center">Fatture</h2>
        <Table
          style={{
            color: "#f1f1f1",
        ...
        >
          <thead>
            <tr>
              {["ID", "Numero", "Data", "Importo", "Stato", "Cliente"].map((header) => (
                <th
                  key={header}
                  style={{
                    borderColor: "#7289da",
          ...
          <tbody>
            {fatture.map((f, i) => (
              <tr key={f.id}>
                {[f.id, f.numero, new Date(f.data).toLocaleDateString(), f.importo, f.statoFatture?.nome || "-", f.clienti?.ragioneSociale || "-"].map(
                  (cell, idx) => (
                    <td
                      key={idx}
                      style={{
